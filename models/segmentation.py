...
class DETRsegm(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        outputs_seg_masks = seg_masks.view(bs, self.detr.num_queries, seg_masks.shape[-3], seg_masks.shape[-2], seg_masks.shape[-1])
...
class MaskHeadSmallConv(nn.Module):
    ...
    def forward(self, x: Tensor, bbox_mask: Tensor, fpns: List[Tensor]):
        ...
        x = cur_fpn + F.interpolate(x, size=cur_fpn.shape[-3:], mode="nearest")
        x = self.lay3(x)
        x = self.gn3(x)
        x = F.relu(x)
